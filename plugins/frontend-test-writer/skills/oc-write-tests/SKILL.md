---
name: oc-write-tests
description: Write Vitest tests for changed code (git diff) or for specific files, using the frontend-test-writer agent
argument-hint: "[FILE... | BASE-BRANCH] (e.g., src/widgets/.../Form.tsx  or  dev)"
---

## Sub-agent Configuration

Use the `frontend-test-writer` sub-agent (via Task tool with `subagent_type: oc-frontend-test-writer:frontend-test-writer`) to write and verify the Vitest tests. This skill only resolves the scope (which files to test) and then delegates everything else — reading the code, writing `*.spec.tsx` / `*.spec.ts`, and running Vitest — to the agent.
...
### Step 3: Invoke the frontend-test-writer Agent

Launch the `frontend-test-writer` sub-agent (`subagent_type: oc-frontend-test-writer:frontend-test-writer`) with a prompt that includes the resolved scope:
...
/oc-write-tests master

# Write tests for one specific file
/oc-write-tests src/srcProject/widgets/b2b/contracts/Form.tsx

# Write tests for several specific files
/oc-write-tests src/srcProject/widgets/b2b/contracts/Form.tsx src/srcProject/widgets/b2b/contracts/mappers.ts
```
