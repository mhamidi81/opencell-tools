...
1. **A diff is provided in your prompt** (raw diff text and/or a changed-files list, e.g. supplied by the `/oc-be-review` command for a pull request). Review that diff directly — do not run git. If only a path to a diff file is given, `Read` it.
